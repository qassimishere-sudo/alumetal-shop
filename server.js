...
// --- مسار الشات المطور (نظام الكروت) ---
app.post('/api/chat', async (req, res) => {
    const { message } = req.body;
    
    try {
        // 1. نجيب البيانات بما فيها الـ ID
        const dbResult = await db.query("SELECT id, title, category, description FROM projects");
        
        let productsContext = "";
        if (dbResult.rows.length > 0) {
            // بنضيف الـ ID في البيانات اللي رايحة للذكاء الاصطناعي
            productsContext = dbResult.rows.map(p => 
                `- ID: ${p.id} | مشروع: ${p.title} (${p.category}) - وصف: ${p.description}`
            ).join("\n");
        } else {
...
معلومات هامة:       
            - نعم، نحن نقدم خدمات الصيانة لكافة أعمال الألوميتال (مطابخ، شبابيك، أبواب) حتى لو الشغل مش من تنفيذنا.

            قائمة مشاريعنا المتاحة (استخدمها للرد):
            ${productsContext}

            التعليمات:
            1. لو العميل سأل عن شغل معين (مثل مطبخ أو شباك) ولقيت مشروع مناسب في القائمة، اشرح "فكرة المشروع" باختصار شديد وتشويق (بدون تفاصيل مملة).
            2. مهم جداً: لو اخترت مشروع معين، لازم تحط الـ ID بتاعه في آخر الرسالة بالشكل ده بالظبط:
            ||ID:رقم_المشروع||
            مثال: "ده مطبخ مودرن ممتاز.. ||ID:5||"

            لو العميل عايز "يتواصل" أو "يكلم حد" أو بيسأل عن "السعر النهائي"، قول له يتواصل معانا واتساب وحط الكود ده في آخر الرسالة:
            ||WHATSAPP||

            3. لو مفيش مشروع محدد، رد عادي بدون الكود ده.
            4. لهجتك مصرية ودودة.
        ...
        res.status(500).json({ reply: "معلش السيستم مشغول، ممكن تكلمنا واتساب؟" });
    }
});
